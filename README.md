...
- **智能弃牌**：开启后机器人使用向听数推荐算法出牌（取 TOP 1 推荐），牌效接近高手水平；关闭则使用伪随机策略，适合日常练习。默认关闭
- **自摸胡**：开启后机器人摸牌若已胡牌则自动胡牌结束对局；关闭时机器人即使胡牌也会继续出牌，不会主动胡牌，适合新手熟悉规则。默认关闭
- **明牌**：开启后游戏过程中可以看到所有机器人的手牌，方便观察对手牌面、学习推算。默认关闭

![机器人策略设置](docs/screenshots/robot-settings.png)
...
- [x] ~~**多套机器人决策算法，支持不同难度。**~~
  已实现智能弃牌（基于向听数推荐算法）与伪随机两种出牌策略，配合自摸胡、明牌等设置，可自由调节牌局难度。
